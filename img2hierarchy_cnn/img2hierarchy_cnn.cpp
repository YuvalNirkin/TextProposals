...
#define VDEBUG 1 // visual debug for manual inspection of intermediate results
...
int main(int argc, char** argv)
{
	...
	float cnn_classifier_threshold = 0.95;	// 0.95
	...
	for (int c = 0; c < channels.size(); c++)
	{
		...
		for (int cue = 0; cue < 5; cue++)
		{
			...
			while (node_idx < dendrogram.size())
			{
				...
				//std::cout << "predictions!!!" << std::endl;// Yuval
				//std::cout << "predictions.size = " << predictions.size() << std::endl;// Yuval
				...
				for (int ib = 0; ib < predictions.size(); ib++)
				{
					...
					///
					//std::cout << "cnn_probability = " << dendrogram[batch_node_indexes[ib]].cnn_probability << std::endl;//
					//std::cout << "cnn_recognition = " << dendrogram[batch_node_indexes[ib]].cnn_recognition << std::endl;//
					///
